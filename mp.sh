...
cmd_status() {
    ...
    if [[ -f "$CFG_FILE" ]]; then
        ...
        echo -e "  VLESS     : $VLESS  |  VMess: $VMESS  |  Trojan: $TROJAN  |  SS: $SS"
    else
...
COLORS = {'vless':C,'vmess':M,'trojan':Y,'shadowsocks':G,'wireguard':W}
...
cmd_uninstall() {
    ...
    echo -e "${YELLOW}[NOTE]${NC} Xray binary and SSL certs kept."
}
...
cmd_help() {
    ...
    echo -e "  ${CYAN}logs [service]${NC}    Tail logs: panel|xray|access"
