...
- یک دامنه برای کانفیگ‌های VPN (مثل `vpn.example.com`)
- یک ساب‌دامنه‌ی جدا برای پنل (مثل `panel.example.com`) که در کلادفلر **DNS only (ابر خاکستری)** باشد و مستقیم به IP سرور اشاره کند — این باعث می‌شود پنل گواهی واقعی بگیرد و مرورگر هیچ هشداری ندهد
- پورت 80 باز باشد (برای Let's Encrypt)
- دسترسی root

### مراحل نصب
نصب‌کننده از شما می‌پرسد:
1. **دامنه VPN** — مثال: `vpn.example.com`
2. **دامنه پنل** — مثال: `panel.example.com` (خالی = همان دامنه VPN)
3. **نام کاربری پنل** — حداقل 4 کاراکتر
4. **رمز عبور پنل** — حداقل 8 کاراکتر

بعد از نصب، پنل روی `https://panel.example.com:9090` با گواهی واقعی و بدون هشدار مرورگر در دسترس است.
